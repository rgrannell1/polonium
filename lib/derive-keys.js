...
const convertToCharset = function (str, charset) {
	...
	while (num.gt(0)) {
		...
		var ith = num.mod(characters.length)

		digits  = digits.concat([ characters[ith] ])
		num     = num.divide(characters.length)
...
convertToCharset.precond = (str, charset) => {
...
// does this need to be syncronous?

const callCrypto = function (master, salt, rounds, bits) {
	return crypto.pbkdf2Sync(master, salt, rounds, bits)
}
